 ...
 * 
 * ENCODER IMPLEMENTATION: VERIFIED & TESTED
 * This code uses the proven encoder reading algorithm from:
 * - Quadrature ISR: Tested to work correctly
 * - RPM Calculation: (deltaCount * 60000) / (interval * CPR)
 * - Both verified with separate encoder test sketch
 * 
 * If encoder still reads 0 RPM after code update:
 *   1. Send: encoder=check
 *   2. Manually rotate motor
 *   3. Check if Encoder Count increases in output
 * 
 * ╔════════════════════════════════════════════════════════════════╗
 ...
 *   encoder=check        → Quick encoder check (manually rotate motor)
 * 
 ...
 *   → Also check "Encoder Count Change" in results:
 *       PWM=0:   Should be ~0 counts
 *       PWM=128: Should be ~93-100 counts
 *       PWM=255: Should be ~187-200 counts
 *   → If all counts are 0: ENCODER NOT READING
 *       Check encoder cable on GPIO 16, 15
 *   test=off
 * 
...
void encoderInterrupt() {
  // Quadrature encoder ISR - reads both channels and determines direction
  // This implementation is verified to work correctly
...
void calculateVelocity() {
  // Get change in encoder count since last calculation
  ...
  // Formula: (deltaCount / OUTPUT_PPR) * (60000 / velocityCalcInterval)
  // This is verified to work correctly
  // Simplified: (deltaCount * 60000) / (velocityCalcInterval * OUTPUT_PPR)
  currentVelocity = (deltaCount * 60000.0) / (velocityCalcInterval * OUTPUT_PPR);
  
  // Optional: Apply moving average filter for smooth velocity
  // (Comment out if raw velocity works better for tuning)
  velocityArray[velocityIndex] = currentVelocity;
...
void runEncoderTest() {
  ...
  velocityIndex = 0;
  currentVelocity = 0;
  previousEncoderCount = encoderCount;  // Reset encoder baseline
  ...
  testPWM = 0;
  long encoderCountAtStart_0 = encoderCount;
  for (int i = 0; i < 30; i++) {
    delay(100);
    Serial.print(".");
  }
  long encoderCountAtEnd_0 = encoderCount;
  speedAt0 = currentVelocity;
  Serial.println("\n  ✓ Speed at PWM=0: " + String(speedAt0, 2) + " RPM");
  Serial.println("  Encoder count change: " + String(encoderCountAtEnd_0 - encoderCountAtStart_0) + " counts");
  Serial.println("  (Should be ~0 RPM and ~0 count change)\n");
  
  // Test 2: PWM = 128 (half power)
  Serial.println("Test 2: PWM = 128 (Half Power)");
  Serial.println("  Waiting 3 seconds for motor to stabilize...");
  testPWM = 128;
  long encoderCountAtStart_128 = encoderCount;
  for (int i = 0; i < 30; i++) {
    delay(100);
    Serial.print(".");
  }
  long encoderCountAtEnd_128 = encoderCount;
  maxSpeedAt128 = currentVelocity;
  Serial.println("\n  ✓ Speed at PWM=128: " + String(maxSpeedAt128, 2) + " RPM");
  Serial.println("  Encoder count change: " + String(encoderCountAtEnd_128 - encoderCountAtStart_128) + " counts");
  Serial.println("  (Should be ~50% of max speed and positive count change)\n");
  
  // Test 3: PWM = 255 (full power)
  Serial.println("Test 3: PWM = 255 (Full Power)");
  Serial.println("  Waiting 3 seconds for motor to stabilize...");
  testPWM = 255;
  long encoderCountAtStart_255 = encoderCount;
  for (int i = 0; i < 30; i++) {
    delay(100);
    Serial.print(".");
  }
  long encoderCountAtEnd_255 = encoderCount;
  maxSpeedAt255 = currentVelocity;
  Serial.println("\n  ✓ Speed at PWM=255: " + String(maxSpeedAt255, 2) + " RPM");
  Serial.println("  Encoder count change: " + String(encoderCountAtEnd_255 - encoderCountAtStart_255) + " counts");
  Serial.println("  (Should be close to 300 RPM and large positive count change)\n");
  
  // Display results
  displayTestResults();
  
  // Display encoder diagnostics
  displayEncoderDiagnostics(encoderCountAtEnd_0 - encoderCountAtStart_0,
                             encoderCountAtEnd_128 - encoderCountAtStart_128,
                             encoderCountAtEnd_255 - encoderCountAtStart_255);
...
void displayEncoderDiagnostics(long countChange0, long countChange128, long countChange255) {
  // ENCODER DIAGNOSTIC ANALYSIS
  // This function checks if encoder is working correctly
  
  Serial.println("╔════════════════════════════════════════════════╗");
  Serial.println("║        ENCODER DIAGNOSTICS                     ║");
  Serial.println("╚════════════════════════════════════════════════╝");
  
  Serial.println("\n🔍 Encoder Count Changes During Tests:");
  Serial.println("  PWM=0:   " + String(countChange0) + " counts (should be ~0)");
  Serial.println("  PWM=128: " + String(countChange128) + " counts (should be ~93-100)");
  Serial.println("  PWM=255: " + String(countChange255) + " counts (should be ~187-200)");
  
  Serial.println("\n🔧 DIAGNOSIS:");
  
  // Check if encoder is reading at all
  if (countChange255 == 0) {
    Serial.println("  ❌ ENCODER NOT READING - Count not increasing!");
    Serial.println("");
    Serial.println("  Possible causes:");
    Serial.println("    1. Encoder not connected to GPIO 16 and GPIO 15");
    Serial.println("    2. Encoder cable loose or damaged");
    Serial.println("    3. GPIO pins configured wrong in code");
    Serial.println("    4. Encoder broken or not rotating");
    Serial.println("");
    Serial.println("  What to check:");
    Serial.println("    □ Unplug motor, manually rotate shaft");
    Serial.println("    □ Send: status");
    Serial.println("    □ Check 'Encoder Count' value");
    Serial.println("    □ Count should increase as you rotate");
    Serial.println("    □ If not: encoder cable is the problem");
    Serial.println("");
    return;
  }
  
  // Encoder is reading, but may have other issues
  Serial.println("  ✓ Encoder is reading (count increased)");
  
  // Check if motor is getting power
  if (countChange255 > 0 && countChange255 < 50) {
    Serial.println("  ⚠️  Motor appears VERY SLOW");
    Serial.println("");
    Serial.println("  Possible causes:");
    Serial.println("    1. Motor power (12V) not connected");
    Serial.println("    2. Motor stalled or jammed");
    Serial.println("    3. Wrong GPIO pins for motor PWM (should be GPIO 5)");
    Serial.println("");
    Serial.println("  What to check:");
    Serial.println("    □ Is 12V power connected to motor?");
    Serial.println("    □ Can you manually rotate motor shaft easily?");
    Serial.println("    □ Try: test=pwm=255 and listen for motor sound");
    Serial.println("");
    return;
  }
  
  // Check linearity
  if (countChange255 > 50) {
    float ratio = (float)countChange128 / countChange255;
    Serial.println("  ✓ Motor is spinning at reasonable speed");
    Serial.println("  ✓ Speed ratio (128/255): " + String(ratio, 2));
    
    if (ratio > 0.4 && ratio < 0.6) {
      Serial.println("  ✓ GOOD - Response is fairly linear");
    } else if (ratio > 0.3 && ratio < 0.7) {
      Serial.println("  ⚠️  Acceptable - Some non-linearity (normal for DC motors)");
    } else {
      Serial.println("  ⚠️  Non-linear response detected");
    }
    Serial.println("");
    Serial.println("  NEXT STEPS:");
    Serial.println("    1. Send: test=off");
    Serial.println("    2. Send: Kp=0.8");
    Serial.println("    3. Send: v=100");
    Serial.println("    4. Open Serial Plotter (Ctrl+Shift+L)");
    Serial.println("    5. Watch and tune!");
  }
}
...
void checkSerialInput() {
  ...
  if (Serial.available()) {
    ...
    if (input == "test=on") {
    ...
    else if (input == "encoder=check") {
      // Quick encoder check without running full test
      Serial.println("\n╔════════════════════════════════════════════════╗");
      Serial.println("║     QUICK ENCODER CHECK                        ║");
      Serial.println("╚════════════════════════════════════════════════╝");
      Serial.println("\nManually rotate motor shaft and watch encoder count increase...\n");
      
      Serial.println("Starting encoder count: " + String(encoderCount));
      long startCount = encoderCount;
      
      Serial.println("Waiting 5 seconds... Rotate motor NOW!");
      for (int i = 0; i < 50; i++) {
        delay(100);
        Serial.print(".");
      }
      ...
      long change = endCount - startCount;
      
      Serial.println("\nEnding encoder count: " + String(endCount));
      Serial.println("Total change: " + String(change) + " counts");
      
      if (change == 0) {
        Serial.println("\n❌ ENCODER NOT WORKING");
        Serial.println("   Count did not change. Check:");
        Serial.println("   - Encoder cable connected?");
        Serial.println("   - GPIO 16 and GPIO 15 correct?");
        Serial.println("   - Encoder powered?");
      } else if (change > 0) {
        Serial.println("\n✓ ENCODER WORKING");
        Serial.println("   Count changed by " + String(change) + " (good!)");
        Serial.println("   Now run: test=on, then test=run");
      }
      Serial.println("");
    }
