...
float kI = 0.0f;   // Usually small or zero
float kD = 0.005f; // Damping term
...
float waveAmplitude = 150.0f;    // RPM (max 150 for 0-300 range)
float waveFrequency = 0.2f;      // Hz
float waveOffset = 150.0f;       // Center RPM
...
void loop() {
  ...
  if ((now - lastPIDTime) >= (SAMPLE_INTERVAL * 1000)) {
    float deltaTime = (now - lastPIDTime) / 1000000.0f;
    lastPIDTime = now;
...
void updateRPM() {
  ...
  if ((now - lastRPMTime) >= SAMPLE_INTERVAL) {
    ...
    // Calculate raw RPM
    float rawRPM = (deltaCounts * 60000.0f) / (SAMPLE_INTERVAL * COUNTS_PER_REV);
...
void updateSetpoint() {
  ...
  switch (currentWaveform) {
    case SQUARE:
      ...
      if (fmod(elapsedTime * waveFrequency, 1.0f) < 0.5f) {
    ...
    case SINE:
      ...
      setpoint = waveOffset + waveAmplitude * sin(2.0f * M_PI * waveFrequency * elapsedTime);
      break;
      
    case RAMP:
      // Linear ramp up and down
      float modTime = fmod(elapsedTime * waveFrequency, 1.0f);
      if (modTime < 0.5f) {
        setpoint = waveOffset - waveAmplitude + (2.0f * waveAmplitude * modTime);
      } else {
        setpoint = waveOffset + waveAmplitude - (2.0f * waveAmplitude * (modTime - 0.5f));
      }
      break;
  ...
  setpoint = constrain(setpoint, 0.0f, (float)MAX_RPM_OUTPUT);
}
...
void pidControl(float deltaTime) {
  ...
  float pidCorrection = (pidOutput / MAX_RPM_OUTPUT) * MAX_PWM * 0.5f;
  
  motorPWM = basePWM + pidCorrection;
  motorPWM = constrain(motorPWM, 0.0f, (float)MAX_PWM);
...
void applyControl() {
  if (setpoint > 50.0f) {  // Dead zone
    ...
    ledcWrite(0, (uint8_t)abs((int)motorPWM));
  } else {
...
void printCSVData() {
  ...
  float elapsed = (millis() - startLog) / 1000.0f;
...
void printInfo() {
  ...
  Serial.print(motorPWM, 1);
  Serial.print(" (");
  Serial.print((motorPWM / MAX_PWM) * 100.0f, 1);
  Serial.println("%)");
