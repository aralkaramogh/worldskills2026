...
void loop() {
  ...
  if (now - lastPrint >= 500) {
    ...
    long deltaTicks = currentTicks - prevEncoderCount;  // Ticks in last 500ms
    prevEncoderCount = currentTicks;
    
    /*
        // RPM = (ticks * 120) / COUNTS_PER_REV
    // (120 = 60 seconds/min * 2 for 500ms interval)
    long rpm = (deltaTicks * 120) / COUNTS_PER_REV;
    */

    long rpm = (deltaTicks * 60000.0)/ (COUNTS_PER_REV * 100); // Alternative RPM calculation for better precision
